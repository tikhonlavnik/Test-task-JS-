...
let db = [
  ...
  const nameCol = document.querySelector(".name"),
    lastNameCol = document.querySelector(".lastName"),
    aboutCol = document.querySelector(".about"),
    eyeColorCol = document.querySelector(".eyeColor"),
    lines = document.querySelector(".lines"),
    fields = document.querySelectorAll(".field");
    // changeColorBtn = document.querySelector(".changeColorBtn");

  let linesArr;
  // основная функция, создающая таблицу. Включает функцию, которая позволяет редактировать эту же таблицу
  function createTable(arr) {
    ...
    lines.innerHTML = "";
    arr.forEach((item, i) => {
      lines.innerHTML += `<div class="line line-${i}"></div>`;
      nameCol.innerHTML += `
      <div class="row-${i} row">${item.name.firstName}</div>
    `;
      lastNameCol.innerHTML += `
      <div class="row-${i} row">${item.name.lastName}</div>
    `;
      aboutCol.innerHTML += `
      <div class="row-${i} row label">${item.about}</div>
    `;
      eyeColorCol.innerHTML += `
    <div class="row-${i} row">${item.eyeColor}</div>
    `;
    })
    linesArr = document.querySelectorAll(".line");
    editRow(arr);
  }

  createTable(db)

  // создание основных кнопок по сортировке таблицы 
  const btnNames = document.querySelector(".btn-names"),
    btnLastNames = document.querySelector(".btn-lastNames"),
    btnAbout = document.querySelector(".btn-about"),
    btnEyeColor = document.querySelector(".btn-eyeColor"),
    arrCopy = db.slice(),
    triggers = document.querySelectorAll(".trigger");

  // удаляет класс выделенных кнопок по сортировке
  ...
  // фильтрует таблицу, получая все значения из основного массива. Создает новый массив, чтобы была возможность вернуть прежние значения
  function filterTable(arr, btn, selector, ...arg) {
    // если присутсвтуют аргументы, то функция запускается для name.firstName или name.lastName, без них для about и eyeColor
    if (arg) {
      arr.sort((prev, next) => {
        if (prev[selector][arg] < next[selector][arg]) return -1;
        if (prev[selector][arg] < next[selector][arg]) return 1;
      });
      fields.forEach(item => {
        item.innerHTML = "";
    ...
    if (btn.classList.contains("toggled")) {
      ...
      arr.sort((prev, next) => {
        if (prev[selector] < next[selector]) return -1;
        if (prev[selector] < next[selector]) return 1;
      });
      pageTable(arr)
      deleteToggled();
  ...
  // навешивание фильтрующей функции на все колонки
   btnNames.addEventListener("click", () => {
    filterTable(arrCopy, btnNames, ["name"], ["firstName"])
  });
  btnLastNames.addEventListener("click", () => {
    filterTable(arrCopy, btnLastNames, ["name"], ["lastName"])
  });
  btnAbout.addEventListener("click", () => {
    filterTable(arrCopy, btnAbout, ["about"])
  });
  btnEyeColor.addEventListener("click", () => {
    filterTable(arrCopy, btnEyeColor, ["eyeColor"])
  });
  // функция необходима, чтобы обработчик событий не привязывался ко всем строкам таблицы и не менял значения всех строк, на которые был совершен клик
  ...
  function removeModalData() {
    ...
    modalWindow.innerHTML = `
      <div class="modal-inner">
        ...
        <input placeholder="Change name" name="name" type="text" class="modal-name modal-win">
        <input placeholder="Change lastname" name="surname" type="text" class="modal-lastname modal-win">
        <input placeholder="Change about" name="about" type="text" class="modal-about modal-win">
        <input placeholder="Change eye color" name="eyeColor" type="text" class="modal-eyeColor modal-win">
        <button class="modal-btn">Edit</button>
      </div>`
    console.log(modalWindow)
  }

  // функция позврляет изменять строки таблицы, рендерит таблицу заново после изменения нужных свойств
  function editRow(arr) {
    linesArr.forEach((item, i) => {
      item.addEventListener("click", (e) => {
        // получение всех инпутов модального окна, с которых будут считываться value
        let modalName = document.querySelector(".modal-name"),
          modalLastName = document.querySelector(".modal-lastname"),
          modalAbout = document.querySelector(".modal-about"),
          modalEyeColor = document.querySelector(".modal-eyeColor"),
          btns = document.querySelector(".btns"),
          btnsActive = document.querySelectorAll(".btn-active");

        // скрытие и последующий показ всех кнопок и фильтров
        lines.innerHTML = "";
        modalWindow.style.display = "block";
        btns.style.display = 'none';
        btnsActive.forEach(item => {
          item.style.display = "none";
        })
        triggers.forEach(item => {
          item.style.display = "none";
        })
        // навешивание события на крестик в модальном окне для его последующего закрытия.
        // Даже если данные были введены, все сбросится до предыдущих значений
        ...
        // считывание и запись всех значений элемента таблицы в соответсвтующие инпуты для последующего редактирования
        modalName.value = arr[i].name.firstName;
        modalLastName.value = arr[i].name.lastName;
        modalAbout.value = arr[i].about;
        modalEyeColor.value = arr[i].eyeColor;
        // сохранение и вывод всех изменений таблицы при нажатии на кнопку edit и сброс всех фильтров
        let editBtn = document.querySelector(".modal-btn");
        editBtn.addEventListener("click", () => {
          ...
          arr[i].name.firstName = modalName.value;
          arr[i].name.lastName = modalLastName.value;
          arr[i].about = modalAbout.value;
          arr[i].eyeColor = modalEyeColor.value;
          deleteToggled();
          pageTable(db);
          removeModalData();
        })
  ...
  // постраничный показ таблицы. Основной массив разбивается на необходимое количество частей, 
  // включающих по 10 строк, чтобы динамически сформировать нужное количество кнопок для переключения на нужную страницу.
  // Таблица рендерится заново с учетом ее редактирования
  function pageTable(arr) {
    let btnsCount = Math.ceil(arr.length / 10),
        btnsArr = [],
        btnsDiv = document.querySelector(".btns");
    let counter = 10;
    // кнопки добавляются в соответствующий массив, после сортировки которого появляется возможность вывода нужной страницы
    btnsDiv.innerHTML = "";
    for (let i = 0; i < btnsCount; i++) {
      btnsArr.push(`<button class="btn">${i+1}<button`);
    }
    btnsArr.forEach(item => {
      btnsDiv.innerHTML += item;
    })
    btnsArr = document.querySelectorAll(".btn");
    btnsArr[0].classList.add("active");
    fields.forEach(item => {
      item.innerHTML = "";
    })
    createTable(arr.slice(0,counter));
    // при переборе всех кнопок в массиве удаляется класс активности у всех кнопок и добавляется той, на которую был последний клик
    btnsArr.forEach((item , i) => {
      item.addEventListener("click", () => {
        ...
        if(i == 0) {
          createTable(arr.slice(i, (i+1)*10))
          btnsArr.forEach(item => {
            item.classList.remove("active")
          })
          btnsArr[i].classList.add("active");
        } else {
          createTable(arr.slice(i * 10, (i+1)*10));
          btnsArr.forEach(item => {
            item.classList.remove("active")
          })
          btnsArr[i].classList.add("active");
  ...
  pageTable(db)
  // эта часть отвечает за показ и скрытие колонок
  let hideBtns = document.querySelectorAll(".btn-active"),
      conts = document.querySelectorAll(".container");

  hideBtns.forEach((item, i) => {
    item.addEventListener("click", () => {
      conts[i].classList.toggle("hide");
      item.style.display = "block";
    })
  });
});